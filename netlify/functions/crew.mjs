...
const pubJob = (j) => ({ id: j.id, name: j.name, trade: j.trade || "", estCost: N(j.estCost), estHours: N(j.estHours) }); // pins never leave the server
  ...
  try {
    ...
    if (req.method === "GET") {
      ...
      if (op === "log") {
        ...
        if (!job || job.pin !== pin) return json({ error: "not found" }, 404); // wrong pin reads as missing
    ...
    if (op === "join") {
      ...
      const jobs = (await store.get("jobs", { type: "json" })) || [];
      const mine = jobs.filter((j) => j.active !== false && j.pin === pin);
      if (!mine.length) return json({ error: "no active job matches that code" }, 404);
    ...
    if (op === "log") {
      ...
      const jobs = (await store.get("jobs", { type: "json" })) || [];
      const job = jobs.find((j) => j.id === jobId);
      if (!job || job.pin !== pin) return json({ error: "not found" }, 404);
